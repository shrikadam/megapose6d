...
if __name__ == "__main__":
    ...
    # Dynamic path construction based on the label
    label_dir = os.path.join(args.base_data_dir, args.label)

    # Auto-generate the paths
    args.video = os.path.join(label_dir, "video.mp4")
    args.camera_json = os.path.join(label_dir, "camera_data.json")
    args.meshes_directory = os.path.join(label_dir, "meshes")
    args.output_video = os.path.join(
        args.base_output_dir, f"final_tracked_{args.label}.mp4"
    )

    if not os.path.exists(args.video):
        raise FileNotFoundError(
            f"Could not find video at {args.video}. check your label or folder structure."
        )
